...
def list_pretrained_models() -> list[ModelRecord]:
    ...
    models = [
        ...
        if model_dir.is_dir()
        if (model := _load_model_record(model_dir)) is not None
    ]
    return models


def find_pretrained_model(model_id: str) -> ModelRecord | None:
    return next((model for model in list_pretrained_models() if model["id"] == model_id), None)


def _load_model_record(model_dir: Path) -> ModelRecord | None:
    ...
    country = COUNTRY_NAMES.get(dataset, dataset)
    ...
    return {
        ...
        "name": f"{model_type.upper()} {country}",
        "country": country,
        "country_code": dataset,
        "dataset": dataset,
        "description": str(metadata.get("description") or "Modelo preentrenado disponible."),
